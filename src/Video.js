...
const server_url = process.env.NODE_ENV === 'production' ? 'https://videocalling-app-vz72.onrender.com/' : "http://localhost:4001"
...
class Video extends Component {
	...
	connectToSocketServer = () => {
		...
		socket.on('user-joined', (id, clients) => {
			...
			// If this is the current socket
			if (id === socketId) {
				// Add local stream and create offer to all peers
		...
		socket.on('set-admin', (isAdmin) => {
			this.setState({ isAdmin }) // Save admin state in component
		})
		...
		socket.on('disable-screen-share', () => {
			this.setState({ screen: false }, () => this.getDislayMedia())
		})
